...
for i in type:
    for j in sp:
        ...
        # stratified_folder = StratifiedKFold(n_splits=4, random_state=0, shuffle=False)
            ...
            conv1 = Conv2D(128, (8, 8), strides=1, padding='same')(x)
            ...
            conv3 = Conv2D(128, (3, 3), strides=1, padding='same')(conv2)
            conv3 = BatchNormalization()(conv3)
            conv3 = Activation('relu')(conv3)
            conv3 = keras.layers.GlobalAveragePooling2D()(conv3)
            # add_0 = Dense(128, activation='relu')(conv3)
            # add_0 = Dropout(0.2)(add_0)
            # add_1 = Dense(128, activation='relu')(add_0)
            # add_1 = Dropout(0.2)(add_1)
            output_layer = Dense(nb_classes, activation='softmax')(conv3)
        ...
        # ------------------------------------------------------------------------------------------------------
            # model.save_weights('F:/TAD_DATA/TAD_CNN_w.h5')
            # model.save('F:/TAD_DATA/TAD_CNN_model.h5')
            scores = model.evaluate(X_test, Y_test, verbose=0)
            cvscores.append(scores[1] * 100)

            # if  scores[1]>temp:
            #     model.save_weights('F:/TAD_DATA/TAD_RRRH_w.h5')
            #     model.save('F:/TAD_DATA/TAD_RRRH_model.h5')
            #     temp=scores[1]
            # else:
            ...
            cm = pd.crosstab(np.argmax(Y_test, axis=1),np.argmax(y_pred, axis=1), rownames=['label'], colnames=['prediction'])

            tn, fp, fn, tp = cnf_matrix.ravel()#改
            tpr = tp / (tp + fn)
            tnr = tn / (fp + tn)
            recall = tp / (tp + fn)
            precision = tp / (tp + fp)
